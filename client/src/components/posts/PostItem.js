        ...
        <div className="post bg-white p-1 my-1">
          <div>
            <Link to={`/developer/profile/${this.props.post.user._id}`}>
              <LazyLoadImage effect="blur" className="round-img" src={avatar} alt={name} />
              <h4>{name}</h4>
            </Link>
          </div>
          <div>
            ...
            <p className="post-date">Posted on {this.getDate(date)}</p>
            <button
              type="button"
              className={`btn ${!auth.loading && auth.isAuthenticated && this.getLikedUnlikeStatus(likes, auth)}`}
              style={{ margin: '15px' }}
              onClick={() => this.props.like(_id)}
            >
            ...
            {displayDiscussionBtn && (
              <Link to={`/post/${_id}`} className="btn btn-primary" style={{ margin: '15px' }}>
