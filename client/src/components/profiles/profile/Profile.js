...
class Profile extends Component {
  ...
  componentDidMount() {
    const dev_profile = this.props.profile.developer_profiles.find(
      (val) => val.user._id === this.props.match.params.id
    );
    if (dev_profile) {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.profile.profile === this.props.profile.profile) this.props.getProfileByID(this.props.match.params.id);
  }
  ...
  render() {
    ...
    return (
      <section className="container">
        <Link to="/developers" className="btn btn-light-outline" style={{ marginRight: '15px' }}>
          Back To Developers
        </Link>
        {!loading && profile && !auth.loading && auth.isAuthenticated
          ? profile.user._id === auth.user._id && (
              <Link to="/edit-profile" className="btn btn-light-outline" style={{ marginRight: '15px' }}>
                Edit Profile
              </Link>
            )
          : ''}
